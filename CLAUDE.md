...
- ⚠️ **Espace client access is decided in ETM, never by the portal** (Clients › Gestion › Contacts, switch per contact, permission `gestion_acces_espace_client`): stored in **PostgreSQL** database `espace_client` (`ESPACE_CLIENT_PG_URL`; dev twin `espace_client_dev`; `lib/espace-client-acces.ts`, migrations append-only) — the first data born outside HFSQL (windev_migration D9: never inside `mps`, which the reload drops). Linked to HFSQL by `idcontact`/`idclient` only. The portal reads `GET /api/site/espace/acces` and reports `POST /api/site/espace/activite`; `espace/*` answers only when the request came through `api-sites.intra…` (they carry customer e-mails; `alpha.etsmalterre.com` still proxies `/api/site`). One e-mail = one access. Per-client routes `espace/clients/:id/*` (fiche, commandes…) return only what the customer may see: each `lib/espace-*.ts` opens with its « never add » list (internal notes, discounts, RIB…) — keep it.
...
- ⚠️ **Every sst line carries a coloris** (#1215): `refuseIfNoColoris()` (400 `coloris_requis`) on line create/edit, « Affecter » and the client-line Tricotage / Ennoblir dialogs; never read `composition_ecru` of a ref without a coloris — the union of all its coloris asked for grey yarn on an écru/écru order.
