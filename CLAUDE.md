...
13. **Agents IA** — Agents (`/agents-ia/agents`, engine `lib/agents/`, state in `data/agents/`, writes gated by `edit_agents_ia`). First agent **BL MATEL** replaces n8n « BL Processing » + the WebDev `load_bl_*`: Mistral OCR + `mistral-small` (`lib/mistral.ts`, `MISTRAL_API_KEY`), mailbox via `lib/gmail-reader.ts`. ⚠️ Modes `off` / `essai` (writes nothing) / `actif`; the poll runs in production only, and a blocking check never writes — the run goes « à vérifier » (`notif_agent_bl`). Checks + benchmark: `screen_notes.md` § 13. Second agent **Superviseur** (`lib/agents/superviseur/`): weekdays 19:00, code checks over ETM + read-only triage of 4 mailboxes (`gmail.readonly`, never the `gmail.modify` client), mails `notif_agent_superviseur` only on NEW findings; ⚠️ only the scheduled run updates the findings memory or sends — « Lancer maintenant » previews. A run is « réussie » unless marked « échouée » with a comment. Deep links `?commande=<id>` on Clients › Commandes and Sous-traitants › Commandes.
