...
import { nowDt, sqlText, resolveMachineLabels } from '../lib/production-trm.js'
...
const STOCK_ECRU_TRM_SELECT = `se.IDstock_ecru, se.IDref_ecru, se.IDcolori_ecru, se.IDordre_fabrication, se.IDpiece_production, se.IDLigne_Commande_TRM, se.poids, se.metrage, se.lot, se.numero, se.num_piece_OF, se.observations, se.visiteur, se.second_choix, se.date_saisie, re.reference AS ref_ecru, ce.reference AS coloris_reference, orf.IDmachine`
...
const STOCK_ECRU_TRM_JOINS = `FROM stock_ecru se LEFT JOIN ref_ecru re ON se.IDref_ecru = re.IDref_ecru LEFT JOIN colori_ecru ce ON se.IDcolori_ecru = ce.IDcolori_ecru LEFT JOIN ordre_fabrication orf ON se.IDordre_fabrication = orf.IDordre_fabrication`
...
async function hydrateTrmEcruRows(rows: StockEcru[]): Promise<StockEcru[]> {
  ...
  const machineLabels = await resolveMachineLabels(fixed.map((r) => Number((r as any).IDmachine) || 0))
  ...
  for (const r of fixed as any[]) {
    ...
    r.machine_nom = machineLabels.get(Number(r.IDmachine) || 0) ?? null
