/**
 * Grandfathering step for the TRM `edit_of` permission.
 *
 *   pnpm --filter @mps/api exec tsx src/scripts/seed-edit-of-trm.ts          # dry run
 *   pnpm --filter @mps/api exec tsx src/scripts/seed-edit-of-trm.ts --write  # persist
 *
 * Production › Ordres de fabrication had **no write gate at all** until now:
 * whoever could open the screen could edit an OF, re-rank a métier's queue and
 * terminate. `edit_of` closes that — and like every TRM key it is **closed by
 * default**, so the day it ships the atelier would lose the screen's writes
 * entirely. This hands the key to everyone who already had the ability, which
 ...
 *
 * ⚠️ **Station accounts are excluded** (`STATIONS` below). A shared workshop PC
 * is precisely what the key exists to hold at read-only: the visitage poste
 * consults the queue and the consigne, it does not terminate OFs. Grant one by
 * hand from Paramètres › Utilisateurs if that ever changes.
 *
