...
// ── Cut-roll dialog ────────────────────────────────────
// Split one physical écru roll into N rolls. The typed pieces are what is CUT
// OFF; the LAST row is auto-computed as the remainder so the weight & length
// totals always equal the original. The remainder stays on the shelf and KEEPS
// the original numero; the cut-off pieces get the next free "<base>-N" numbers
// from the API's cut preview (LIVA #1135, same rule as Finis › Stock).
...
function CutRollDialog({
  ...
  const previewQuery = useQuery<{ base: string; next: number }>({
    queryKey: ['stock-ecru', 'cut-preview', origId],
    queryFn: () => apiFetch(`/stock/ecru/${origId}/cut/preview`),
