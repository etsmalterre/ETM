...
|---|---|---|
| `/new-feature-worktree <name> [ng\|trm]` | ETM main checkout | allocate slot for the project (default `ng`), create worktree off that repo's `origin/master`, `pnpm install`, wire dev env (ng: `.env.development` CORS + `secrets/`, start API+web; trm: `.env.development.local` `VITE_API_URL`, start web only), health-check, register. Then open a new Claude in the worktree. Since 2026-09-15 the skill passes `--terminal`: a « free » Windows Terminal window of the 2x3 grid is replaced, on the same spot, by one titled after the feature that runs the context launcher (`yolo-ets` / `yolo-liva`) in the worktree, and turns back into « free » when Claude exits — `C:/dev/claude_config/bin/wt-slot.ps1` (`list` / `claim` / `free` / `layout`). On Linux inside Herdr (since 2026-09-17) the worktree becomes a tab of the repo's Herdr space, named after the feature, and the launcher runs in its shell; the Claude SessionStart hook (`claude_config/config/hooks/task-line.mjs`) puts the task line and the web URL on that agent's sidebar row. Outside Herdr a new terminal window is opened on the current Hyprland workspace. |
