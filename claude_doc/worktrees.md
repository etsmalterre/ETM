...
- **Deploy ownership**: `/etm_deploy` ships the shared API (+ NG web) to
  `etm.intra.etsmalterre.com`; `/trm_deploy` ships only the TRM web to `trm.intra.etsmalterre.com`
...
|---|---|---|
| `/new-feature-worktree <name> [ng\|trm]` | ETM main checkout | allocate slot for the project (default `ng`), create worktree off that repo's `origin/master`, `pnpm install`, wire dev env (ng: `.env.development` CORS + `secrets/`, start API+web; trm: `.env.development.local` `VITE_API_URL`, start web only), health-check, register. Then open a new Claude in the worktree. The skill passes `--terminal`: run from a Herdr pane (`HERDR_ENV=1`, the normal case on every platform since 2026-09-21; Linux since 2026-09-17) the worktree becomes a tab of the repo's Herdr space, named after the feature, and the context launcher (`yolo-ets` / `yolo-liva`, the bash function or its `.ps1` twin on Windows) runs in its shell, so a « <repo> · <feature> » agent row appears in the sidebar; the Claude SessionStart hook (`claude_config/config/hooks/task-line.mjs`) puts the task line and the web URL on that row. Outside Herdr only: on Windows a « free » window of the 2x3 Windows Terminal grid is claimed (`C:/dev/claude_config/bin/wt-slot.ps1`, retired from daily use), on Linux a new terminal window opens on the current Hyprland workspace. |
