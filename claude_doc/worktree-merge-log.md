# Worktree merge log
## 2026-09-25 — feat/espace-commandes
**The customer's orders for the espace client** (etsmalterre-site « Mes commandes »). New site-webservice routes under \`espace/\` (api-sites host only): \`GET espace/clients/:id/commandes\` (every order of the client, newest first: numero, date, ref_client, status, lines, Σ HT, next planned ship date, Malterre references — ≈ 0,4 s for 476 orders; shipped quantities only worked out for open orders), \`GET …/commandes/:idCommande\` (header comment, delivery/billing addresses, mode de paiement, échéance, remise, frais de port, lines with coloris / divers variations / qty / unit / price / planned ship date / shipped / status, shipments with rolls-ml-kg or colis, definitive invoices + credit notes via ligne_facture → ligne_expedition and facture.IDexpedition_divers), \`GET …/:idCommande/confirmation.pdf\` (the Clients › Commandes « PDF »). Logic in \`lib/espace-commandes.ts\`, reusing the Clients › Commandes helpers (now exported: resolveLineLabels, lineReservationAggregates, diversShippedByArticle, renderClientPdfBuffer…). Status (Vincent): En cours / Partiellement expédiée / Expédiée / Soldée from what shipped, a line counting as shipped at ≥ 95 %; shipped = max(rolls reserved to the line and shipped, rolls on its shipment lines — legacy shipments never reserved them). Excluded: internal log, billing notes, fiche client, line comments, subcontractors, reservations, proformas, donation orders, shipments with no roll yet. ETM screens unchanged.
## 2026-09-25 — feat/asso-ref
**Côte priced at its molleton's band + associated refs editable on Finis › Références** (LIVA #1217, Isabelle, Lemahieu order N° 3891). Finis › Références gains a Classeur tab « Associées » over `ref_fini.associee` (CSV of IDref_fini, never editable in ETM before): add/remove persist immediately (`lib/refs-associees.ts`, `POST/DELETE /references-fini/:id/associees`, serial lock, CSV rewritten clean); removing one a client still ticks answers 409 `association_utilisee` naming the clients (Vincent: block and list). Clients › Commandes: `calcLignePriceClient` takes `IDcommande_client` / `IDligne_commande_client`; a fini line whose ref the client links under a parent designation (`designation_client.associee` → hidden « Reference Associée » child) takes the best roll band of the parent lines of the same coloris on the order when better than its own (`lib/palier-associe.ts`, port of legacy `COMBO_Associée_a`). « Same coloris » = `colorisKey()` — names without the dyer lab number / punctuation / accents (prod 2026-09-25: 738/861 côte lines identical names, +41 with the key). The line dialog shows « Tarif au palier N rouleaux du molleton X » and drops the Tricobot nudge. After a molleton line is added/changed/deleted, `GET /commandes-client/:id/cotes-a-revoir` lists paired côtes whose auto price now differs (invoiced ones skipped) and `RepriceCotesDialog` applies them via `POST` only on « Mettre à jour », re-priced server-side. Roll geometry moved to `lib/roll-geometry.ts`. Also: the line dialog shows the stored REAL price rounded to the centime (was 13.359999…). Not done: devis.
## 2026-09-25 — feat/compo-affectation-1215
**A sous-traitant order line always names its coloris** (LIVA #1215, Pierre-Emmanuel: order 9062, 329 écru for Tricotage Malterre, « MPS veut que j'affecte du gris »). The line had been saved without a coloris, and the Stock fil drawer then read every `composition_ecru` row of the écru — the union of its four coloris plus the colourless recipe no lot matches — so « Affecter » could never appear. API: `refuseIfNoColoris()` (400 `coloris_requis`, French message) on `POST /commandes-sous-traitant/:id/lignes`, on `PUT /lignes/:id` whenever the patch touches reference or coloris (date / statut-only patches still pass for old lines), on `POST …/affecter`, and on the client-line `supply/tricotage/orders` and `supply/ennoblissement/orders`; `loadCompositionPairs` returns nothing without a coloris and the drawer payload carries `coloris_manquant`. Web: « Coloris * » with no « Aucun » row, Enregistrer disabled with the reason as its title, a note when the reference has no coloris, the server message under the line form (`InlineForm` gained `saveBlockedReason` + `error`), a « pas de coloris » banner in the drawer; the Tricotage / Ennoblir dialogs of Clients › Commandes now show the server's message instead of « API 400 ». Prod: line 9035 set to ecru/ecru by Vincent. Docs: `screen_notes.md` § 4.
## 2026-09-25 — feat/contexture-tm-1216
**Tombé Métier › Stock : colonne Contexture + critère « Contexture : »** (LIVA #1216, Pierre-Emmanuel). `attachEcruLabels` (`routes/stock-ecru.ts`) now also resolves `contexture_nom` without a JOIN: the `ref_ecru` lookup reads `IDcontexture` in the same query, then one more flat `IN` lookup on `contexture` with the batched accent repair — one extra small query per list/detail hydrate. Web (`TombeMetierStock.tsx`): sortable Contexture column (widths rebalanced to 100 %: Coloris / Lot / Client / Visiteur / Magasin / Observations / Défauts each −1 %), a `Contexture` field in `SEARCH_FIELDS` (the « filtre » is a SmartSearchInput chip, per the #1156 doctrine — no new dropdown), free-text haystack, mobile card and drawer subtitle. Dev copy: 762 available pieces, ten contextures, only ref 472/1 has none (« — »); « Contexture : vanisé » → 297 pieces = the four vanisé contextures. Docs: `screen_notes.md` § 7.
...
## 2026-09-25 — feat/client-ca-1214
Tableau de bord › Évolution du CA per client (LIVA #1214, Isabelle: the legacy "détail du CA" followed one client month by month). `GET /rapports/ca-evolution?client=<IDclient>` narrows `caForYear` in SQL (new optional `clientId` arg, `AND f.IDclient = …`, ~0,1 s) — agrees with the CA table to the centime (Le slip Français 2024/2025 checked); the unfiltered call also returns `clients` (billed over the window) for the picker. Web: `SearchableCombobox size="sm"` in the widget's navy header beside Mensuel / Annuel (new optional `inputClassName` prop on the shared combobox, here `text-left`), ✕ back to all clients; for one client the current year is cut at the calendar month; the axis goes below 0 only when a negative month would show (> 2 % of max), smaller dips drawn on the zero line. TRM's own copy of the widget does not have the picker yet (endpoint ready for both sociétés). Docs: `dashboard_widgets.md` § Évolution du CA.

## 2026-09-25 — feat/resolu
Agents IA › Superviseur report (web only + dev seed). Sections are now « Nouveaux points » / « Toujours ouverts » / « Résolus »: a point Isa marks résolu moves to « Résolus » immediately instead of staying under « Toujours ouverts » (the KPI « n nouveaux · m ouverts » excludes them; « Aucun point à traiter » shows once everything is dealt with). Cards redesigned for reading: title + one meta line, context in grey, the mail check's « À faire : » split into a gold callout, a zinc footer strip gathering résolution, score and « Marquer résolu »; résolu cards fold to title + explanation. Every coloured left edge on the Agents IA screen standardised on the thick 4px (class order `border border-l-4` under `cn()`). `seed-superviseur-dev.ts` mail 1 now carries an « À faire » part. Docs: `screen_notes.md` § 13 « Report layout ».

## 2026-09-25 — feat/superviseur
Agents IA › Superviseur: « résolus » explained, « Marquer résolu », and v2 (LIVA #1212, from Isabelle's first scores — ETM checks 5 réussites + 1 partielle, mail check 1 / 5 / 4 échecs). **Résolus**: a closed point says why — every check records `ctx.raison(cle, texte)` for what it lets pass (« Réponse de pierre-emmanuel le 24/09 », « Ligne couverte : 950 Ml… »), `Controle.raisonAbsent` when the object left scope; a person can « Marquer résolu » a point with a mandatory explanation (`PUT /agents-ia/:slug/runs/:id/points/resolution`, `data/agents/superviseur-resolutions.json`) — listed under « Résolus » while the check still returns it, not a score, never « à évaluer », shown in « Retours ». `Constat.empreinte` (mail: the client's last Message-ID): a new client message makes the point new again and drops an earlier score / résolu. **v2 mail check**: scope in code (Nicolas's mailbox only with Isabelle in copy, Isabelle's inbox after 5 working days — defaults chosen without her answer, constants in `boites-liste.ts` / `regles.ts`); ETM checks in `verifications-etm.ts` (requested document already in `envoi_email` after the message → closed, before → `info` with dates; ETM sending a CITED document → answered; announced address already in `adresse` → closed); prompt v2 with a never-reported `technique` category, offered in the Prompt tab via `AgentDef.promptLivre` (« Publier la v2 », never auto-published — **publish it after deploy**). THUASNE (IDclient 52) couverture points become `info` (partial-delivery agreement). Prod replay: 3 mail points instead of 10. Dry run gained `--prompt=livre --raisons`. Docs: `screen_notes.md` § 13 « Résolus » + « Superviseur v2 ».

## 2026-09-25 — feat/permission-param
**Paramètres joins the Écrans axis; `import_compta_sage` removed** (paired with TRM `feat/permission-param`). Rule from Vincent: Écrans decide what a user SEES, permissions what they can DO in it — a new screen gets an Écrans entry, a key only when viewers of the same screen need different rights. Paramètres is now a menu of the axis (`screen_settings`, Outils hideable via `hide_settings_outils`) in both `screen-keys.ts` and `screen-keys-trm.ts`, with `seed: false` so the grandfathering scripts never hand it out (and the Écrans tab's « Tout » doesn't tick it); Utilisateurs stays `adminOnly` and out of the tree. Web: `screenAccessMenus()` / `visibleSettingsItem()` in `navigation.ts`, `useVisibleSettingsItem()`, AppShell's guard now covers `/settings/*`, `SettingsIndex` and `SettingsOutils` read `useScreenAccess()`. API: `routes/import-sage.ts` checks `userCanOpenScreen` / `trmUserCanOpenScreen` (menu grant + no hide, admin bypass) — the curtain is the lock here, same shape as TRM Pointage. Manifest guards parse `settingsItem` (admin-only entries skipped). Not deployed before the change, so no stored grant to migrate; after deploy, grant Isabelle Paramètres in Écrans (ETM and TRM).

## 2026-09-25 — feat/sage
**Paramètres › Outils › Import de la balance Sage, ETM + TRM** (paired with TRM `feat/sage`). Port of the legacy `FEN_upload_compta`, the weekly import feeding Rapports › Finance and every finance widget; the WinDev window was still its only writer (73 ETM + 74 TRM uploads, last 2026-09-21). Rules recovered from the compile cache and replayed on the 147 exports the legacy stored in `upload_compta.fichier` (`scripts/check-import-sage.ts`, read-only on prod: totals exact on all TRM + all ETM 2026 uploads; ETM 2024-25 differ by exactly the unported stock step). API: `lib/import-sage.ts` (parser, totals, company fingerprint, 16 tests) + `routes/import-sage.ts`, one factory mounted `/api/outils/import-sage` (société 1) and `/api/outils-trm/import-sage` (société 2); new key `import_compta_sage` in both catalogs (reads gated too — payroll accounts). Preview `POST /analyse` writes nothing; import refuses a file resembling the other company (`mauvaise_societe`, ≥ 33-point margin on history), a second import the same day, an unreadable file, a duplicated account; header written last, orphan releves purged on retry; only the latest import can be deleted. The legacy `PriseEnCompteStockETM` is NOT ported (decision: `variation-stock.ts` estimates 603700) — only its weekly 0 € row on 603700 is kept, which the finance screens need to show the estimate. Web: `SettingsOutils.tsx` (Tableau history + §18.D banded preview), rendered by TRM through `@etm` with its `basePath`; `SettingsIndex.tsx` sends `/settings` to the first Paramètres screen the viewer may open; Sidebar / MobileNav now filter Paramètres through `useSubmenuFilter` (they ignored `permission` entries, and mobile always showed the menu). Tested end to end on dev (import 81 releves like legacy, refusals, byte-identical download, undo). **To do at go-live**: grant « Importer la balance Sage » to Isabelle in ETM AND TRM; retire the WinDev button.
...
## 2026-09-24 — feat/etiquette-1200
Simone Pérèle roll labels (LIVA #1200), port of the legacy FEN_Etiquettes_SP / FEN_Codes_SP / ETAT_Etiquette_SP. Clients › Commandes: new « Étiquettes » tab in a fini line drawer (EtiquettesSpTab), shown for clients switched on in Clients › Gestion (Info switch « Étiquettes code-barres par rouleau » + « Étiquettes » master tab managing code_sp, CodesSpTab). Order number, coloris → code_sp, rolls, MATEL lot and net come from ETM; MATEL measures (brut, net, laize cm, tare, poids) are auto-saved per roll in data/etiquettes-sp.json (JSON store, no new HFSQL table). Two PDFs, printed or emailed to the sous-traitant holding the rolls: TableauMetragePdf (the sheet MATEL fills) and EtiquettesSpPdf (4 labels per A4 landscape, legacy layout). New router /api/etiquettes-sp (routes/etiquettes-sp.ts), Code 128 / GS1-128 encoder lib/gs1-barcode.ts (tests pinned to the legacy PDFs' SSCCs and order barcode). Legacy GS1 barcodes had no start character (unscannable) → now real GS1-128, same content; SSCC = 9999999 + roll number + key. 26/54 code_sp EANs are 11 digits → labels refused (409 ean_invalide) until fixed. Also: the order line drawer's tab strip now scrolls sideways when narrow (close button pinned). Docs: CLAUDE.md § 3 Clients, screen_notes.md § 3.

## 2026-09-24 — feat/ech55-1205
TRM ticket LIVA #1205, API + shared Tombé Métier › Références screen. « ech 55 » (ref_ecru 540), duplicated from 029, kept 029's per-coloris `composition_ecru` rows under an edited reference recipe, and TRM commande 2908 offered 029's cotton: every yarn-picking reader prefers the coloris rows, which the screen never showed. The Coloris card now lists each coloris's own recipe (or « Suit la composition de la référence »), a « ≠ composition » badge on the coloris and a count in the card header when its yarns or shares differ from the reference's (`compositionEcart`, yarn colours ignored, positions of one yarn summed), and in edit mode a flask button opens `ColorisCompositionDialog` (yarn, yarn colour from new `GET /references-ecru/lookups/colori-fil?ref_fil=` with stock kg to tell same-named colours apart, %, « Reprendre la composition de la référence »). Saved whole by `PUT /references-ecru/:id/coloris/:coloriId/composition` (empty = follow the reference, total 100 %, yarn colour must belong to its yarn, row ids reused in order by `planColorisComposition`, 409 if a removed row is named by a liage chute). Lock per coloris, narrower than the ref's (Vincent's decision): rolls or OFs in that colour only. Detail carries `composition` / `composition_ecart` / `composition_lock` per coloris. Pure helpers + tests in `lib/composition-coloris.ts`. Prod ech 55 repaired by hand the same day (ecru = PA66 30 / lyocell 64 / élasthanne 6, the five 029 coloris removed). 14 prod refs had a diverging coloris, mostly with rolls — left as is.
...
## 2026-09-24 — feat/pointage — « 7 derniers jours » de la tablette pointage + règle d'avance / de dépassement
**MPS API (paire TRM `feat/pointage`, apps/pointage).** La boucle par jour du rapport de pointage email sort de `construireRapportPointage` en **`analyserJours(jours, idSalarie?)`** (`lib/rapports-pointage-envoi.ts`), partagée par l'email et la nouvelle route tablette **`GET /api/pointage/salaries/:id/journees`** (même garde que `/etat`) : les 7 derniers jours travaillés d'un salarié (ligne pointée ou ligne de planning, 30 jours de recherche, jusqu'à hier), jugés par les règles de l'email. Helpers purs `joursPrecedents`, `derniersJoursTravailles` (testés). **Nouvelle règle** (`DEBORD_MAX_MIN = 10`, Vincent) : arriver plus de 10 min en avance ou partir plus de 10 min après l'horaire est signalé en rouge comme un retard (temps de recouvrement payé pour rien), début et fin de journée seulement — l'email la porte aussi. Seed de dev `scripts/seed-journees-pointage-dev.ts` (8 jours de semaine + planning des postés, chaque défaut au moins une fois, refuse une base non locale). Un premier essai côté ERP (`/pointage-admin/salaries/:id/journees`) a été retiré le jour même.

## 2026-09-24 — feat/superviseur — guide de notation, coûts en €, boîte d'Isabelle
**MPS API + web ETM.** **Définitions des notes** réécrites autour d'une seule question qui tranche, par agent (`AgentDef.evaluation` + nouveau `guide` : question, un exemple par note, remarques ; servi en `guideNotation` par `GET /agents-ia`). Superviseur : « Si j'avais ignoré ce point, est-ce que ça aurait posé un problème ? » (vrai mais inutile = échec). BL : « Est-ce que je garde ce qu'il a écrit ? » (tel quel / en corrigeant / je jette tout). À l'écran, **un seul bouton « Comment noter ? » en haut du rapport** (à gauche de la ligne au-dessus des points), qui déplie le guide en tête de liste — jamais un par point ; le panneau de note BL garde son lien sous les boutons. La phrase « Évaluez chaque point… » est retirée. **Coûts IA en €** (`fmtEur`, taux indicatif fixe `EUR_PER_USD = 0.86`, « < 0,01 € » sous le centime) au lieu des m$. **Le Superviseur lit aussi `isabelle@etsmalterre.com`** (5 boîtes, `boites-liste.ts`, lecture vérifiée par `essai-superviseur-boites.ts`) : une réponse d'Isabelle compte désormais comme réponse au client. Nouveau `scripts/seed-superviseur-dev.ts` : rapport de test en dev (contrôles base réels, points mail d'exemple, aucune boîte lue, refuse une base non locale).

## 2026-09-23 — feat/agents-kpi — Superviseur : le rapport se lit en KPI, l'agent se note par ses points
**MPS API + web ETM.** Décision du soir : pour le Superviseur, **l'unité de travail est le point, pas le rapport** — le rapport n'est que la fournée du matin. **Plus de notation du rapport** (`PUT /:slug/runs/:id/evaluation` → 409 pour un agent `pointsEvaluables` ; le panneau « Évaluer ce rapport » disparaît, les quatre rapports notés avant restent tels quels dans le fichier, sans affichage). **Le dialogue du rapport** : la boîte de chiffres à droite est remplacée par une **bande de KPI en haut** (signalés · évalués avec barre de progression · confirmés · partiels · fausses alertes · résolus — un chiffre n'est coloré que quand il porte un sens, règle §18.D à plat), durée et « lancement manuel » remontent dans l'en-tête, les cartes de points prennent toute la largeur, un point déjà noté se replie sur sa ligne de note + « Modifier » (les trois boutons n'apparaissent que là où une décision reste à prendre), « Afficher les contrôles exécutés » reste en lien. **L'état d'un rapport dérive de ses points** (`bilanRun()` dans `lib/agents/superviseur/score.ts`, pur, testé : un point noté sur un rapport précédent compte comme noté) — colonne « Évalués » de la liste (`évalués / signalés` + une icône par note), filtres `?note=` lus à travers les points. **Le score de l'agent** est celui de ses points (`scorePoints()` : chaque constat distinct que la version a signalé, écartés compris, par son dernier avis) avec une **précision = (confirmés + partiels) / évalués** — carte « Version N — points » dans l'Aperçu (vert ≥ 80 %, ambre ≥ 50 %). Les textes des trois notes d'un point viennent du catalogue (`AgentDef.evaluation`) au lieu d'être codés dans l'écran. BL Ennoblisseur inchangé (une exécution = un BL, notée telle quelle).

## 2026-09-23 — feat/agents — notation des exécutions, BL MATEL → BL Ennoblisseur, Superviseur en rapport du matin
**MPS API + web ETM.** Agents IA reçoit une **notation de chaque exécution** : réussite / partielle / échec (`AgentRun.evaluation`, `PUT /:slug/runs/:id/evaluation`, commentaire obligatoire hors réussite → 400), gardée par un nouveau droit **`evaluer_agents_ia`** distinct du pilotage `edit_agents_ia` (Isabelle note sans pouvoir modifier un agent — à accorder dans Paramètres › Utilisateurs après déploiement). Les anciens `verdict` (pouces) sont lus comme des notations par `normaliserRun()`. Un **échec retire ce que l'exécution a écrit quand l'agent le déclare** (`AgentDef.evaluation.retirer`) : BL Ennoblisseur retire ses lignes `data_bl_tricotbot` (ids enregistrés à l'écriture, `Ecriture.ids` ; anciennes exécutions rapprochées sur ligne + lot + pièce + poids + métrage), **jamais le PDF** ; `ConfirmDialog` avant. Nouvel onglet **« Retours »** (`GET /:slug/retours?version=`) : tous les commentaires d'une version, repris dans « Nouvelle version » pour écrire le prompt suivant ; stats par version réussites / partielles / échecs / à évaluer. **BL MATEL renommé BL Ennoblisseur** (slug `bl-matel` → `bl-ennoblisseur`, fichiers `bl-ennoblisseur(-db).ts` ; `SLUGS_RENOMMES` dans `store.ts` déplace `runs-bl-matel.json` et l'état à la première lecture — aucune migration manuelle en prod ; lit toujours MATEL seulement). **Superviseur** : passe de 19 h à **05 h** les jours ouvrés et **n'envoie plus de mail** — l'exécution EST le rapport du matin lu dans Agents IA (`notif_agent_superviseur` et `superviseur/email.ts` supprimés, statuts `points_a_voir` / `rien_a_signaler`, modes `off` / `actif` seulement). Chaque point du rapport se note (`avisPoints`, `PUT /:slug/runs/:id/points`, mémoire par clé de constat dans `data/agents/superviseur-avis.json`, `superviseur/avis.ts`) : un échec = fausse alerte, écartée des rapports suivants tant que le contrôle la renvoie. Boîtes mail lues en `gmail.readonly` (`superviseur/boites.ts`), jamais le client `gmail.modify`. **« Relever / Lancer maintenant » ne bloque plus** : `POST /:slug/sonder` répond 202 avec un `lancement` (`lancerSondage()` dans `scheduler.ts`, testé) et l'écran interroge `GET /:slug` jusqu'à la fin — la version synchrone tombait en 504 nginx à 60 s sur un run Superviseur de ~1 min. Tests ajoutés : `scheduler.test.ts`, `store.test.ts` (520 tests API, 38 web au vert). Liens profonds `?commande=<id>` sur Clients › Commandes et Sous-traitants › Commandes.
...
## 2026-09-18 — feat/probleme-pieces (doublons de pièces au transfert, LIVA #1172)
**Transferts › Rouleaux / expédition TRM** : trois rouleaux TRM (3568/7, 3573/13, 3574/10) transférés à MATEL le 14/09 avant leur expédition TRM (le trou #1169, fermé le 17/09) ont été remis à l'usine (`IDmagasin = 0`) le 17/09 à 09:15 par le « Expédier » du **legacy WinDev** encore ouvert sur PC-PIERROT — identifié dans le journal HFSQL `stock_ecrujnl` (User 84 = `MPS.exe`, procédure de lecture dans `hfsql_odbc.md` § « Reading the HFSQL journal »). Le picker les a reproposés, d'où les doublons sur les BL 4441–4443 ; « Retirer » les a renvoyés au magasin source (l'usine) — comportement nominal, pas une suppression. Réparation prod à la main (`IDmagasin = 9` sur les trois). Le legacy est désinstallé ; l'API tamponne désormais elle-même `IDref_commande_source` à l'expédition TRM d'une ligne miroir (`lib/trm-handover.ts` : `handoverSets()` / `releaseSets()`, utilisés par `stampShippedPieces` / `releaseShippedPieces`, 5 tests) et ne touche jamais `IDmagasin`. Rattrapage exécuté en prod : `scripts/repair-1172-source-line.ts --write` (149 rouleaux expédiés par l'API sans ligne source, 27 natifs TRM laissés à 0). Docs : règle dans `CLAUDE.md` (tampon + lecture du journal), post-mortem `screen_notes.md` § 5, correction du modèle sst (« stamped at shipping », pas au visitage). Réponse développeur envoyée sur le ticket.

## 2026-09-18 — feat/factures (« Marquer comme envoyée » sur une facture définitive, LIVA #1174)
**Clients — Facturation** : le cadre rouge « non envoyée » ne lit que l'existence d'une ligne `envoi_email` (type 19), écrite jusqu'ici par le seul `POST /:kind/:id/email` — une facture partie autrement (PDF complété pour la douane, envoi groupé depuis Gmail) restait rouge. Nouveau bouton `MailCheck` dans le trio d'en-tête (définitive rouge, permission `edit_factures`) → dialogue avec motif obligatoire → `POST /def/:id/marquer-envoyee` écrit une ligne-marqueur sans adresse, `notes = "marquee envoyee hors application|<auteur>|<motif>"` (`lib/envoi-manuel.ts` + test). L'onglet Historique rend « Envoyée hors application · Par X · motif » avec un « Annuler » → `DELETE …/marquer-envoyee` (retrait par préfixe ASCII, un vrai envoi enregistré entre-temps garde la facture verte). `GET /:kind/:id` renvoie `est_envoye` via le `loadEnvoyeIds` partagé avec la liste ; `logEnvoiEmails` et le nom d'utilisateur factorisés. Proforma → 404. Vérifié sur la copie de dev (cycle marquer / relire / annuler par curl, puis dans l'écran). Envoi groupé non retenu. Réponse développeur envoyée.

## 2026-09-18 — feat/total-affecte (une pièce donnée ne compte plus dans la jauge « Affecté », LIVA #1173)
**Clients › Commandes** : la commande 3795 THUASNE lisait « Affecté 192,6 / 200 Ml » pour trois rouleaux finis totalisant 162,8 ; les 29,8 Ml étaient l'écru 1448/17 (10 kg de 2e choix de 2022) attaché à la donation permanente 2071 ET encore réservé à la ligne Thuasne (10 × rendement 2,97767), invisible sur tous les onglets puisque tout lecteur de stock masque une pièce donnée depuis #1154. Les deux FK `IDcommande_donation` / `IDligne_commande_client` sont indépendantes et les fenêtres WinDev écrivent chacune la sienne ; les 50 pièces trouvées en prod (24 écru, 26 finis) sont toutes sur des commandes antérieures au go-live. Changements : `attachDonationSql()` met `IDligne_commande_client = 0` dans le même UPDATE (écru et fini), `lineReservationAggregates()` exclut les pièces données sur les deux catalogues, les quatre routes d'affectation répondent 409 `piece_donnee` (`refuseIfDonated()`), garde + réparation `check-donation-lien-client.ts [--repair]` — lancée en prod depuis le laptop (connexion pointée sur `10.10.20.2`) le 18/09/2026 : 50 / 50 réparées, re-contrôle OK, la jauge de 3795 lit 162,8. Tests `donation-pieces.test.ts` (+3, round-trip adapté). Règle dans `CLAUDE.md` § Data semantics, récit dans `screen_notes.md` § 3. Réponse développeur envoyée sur le ticket.

## 2026-09-18 — feat/coloris (coloris manquant sur les rouleaux 095A, LIVA #1175)
**Finis › Stock** : réparation de données, pas de changement de code applicatif. Les deux rouleaux 095A en stock (3544/1 et sa coupe 3544/1-1) avaient `IDColoris = 0` — réceptionnés avant le correctif #1158, sur une ligne ennoblisseur sans coloris. Nouveau script `apps/api/src/scripts/repair-1175-coloris-fini.ts` (dry run par défaut, `--write`) qui pose sur un fini sans teinture le `colori_ecru` de la pièce source (ou l'unique coloris de l'écru) et refuse de deviner ailleurs ; appliqué en prod le 18/09 (2 rouleaux). Post-mortem dans `screen_notes.md` § 8.

## 2026-09-17 — feat/bug-expe (le picker de transfert ne propose que les rouleaux d'ETM, LIVA #1169)
**Transferts — Rouleaux** : `ecruWhere` porte `se.IDsociete = 1` (`ECRU_OWNED`), donc la liste `/available` et ses deux compteurs, et la re-vérification de `PUT /:kind/:id/pieces` pose la même condition. Les rouleaux que TRM vient de visiter sont à `IDmagasin = 0` comme ceux d'ETM et ne passent en société 1 qu'à l'expédition TRM : sans filtre, l'usine les listait dès leur sortie du métier et Pierrot a mis 3568/29–31 et 3573/23 sur le bon 4441 vers MATEL avant que TRM n'expédie (l'avis TRM 12329 les affichait « MATEL »). Vérifié sur la copie de dev : l'ancien prédicat offrait 6 768 rouleaux non-ETM à l'usine, le nouveau aucun. Garde (e) ajoutée à `check-transfert-picker-search.ts` (le script s'arrête avant sur la copie de dev, faute d'une référence à deux coloris). Aucune modification TRM : le worktree apparié n'a rien à livrer. Réponse développeur envoyée sur le ticket.
...
## 2026-09-15 — feat/enrollement (« Enrôler ce téléphone » seulement quand un code attend)
**Atelier PWA — enrôlement** : nouvelle route publique `GET /api/atelier/appareils/enrolement-en-attente` → `{ enAttente }`, vrai tant qu'un code d'enrôlement émis par un admin n'est ni consommé, ni expiré, ni annulé (`codeEnAttente()` dans `lib/appareils-atelier.ts`, testé). Un booléen seulement — jamais le code, le libellé ni le régleur ; le frein brute force de `/enroler` reste la garde. Branche TRM appariée (`feat/enrollement`) : l'Accueil de la PWA n'affiche le lien « Enrôler ce téléphone » que pendant qu'un code attend (poll 10 s, tant que le téléphone n'est pas enrôlé), et l'affiche aussi sur erreur (API pas encore déployée, réseau) pour ne jamais fermer la seule porte. Vérifié sur l'API du worktree : faux → vrai après `POST /codes` → faux après `DELETE /codes/:code`.
...
## 2026-09-14 — feat/piece-groupe (LIVA #1149 — Pièces regroupées : « Fusionner » à la réception ennoblisseur)
**#1149 Sous-traitants › Commandes › Réception** : MATEL coud parfois deux pièces écru en un seul rouleau teint (petites pièces → moins de coutures, blanc pour impression) ; le BL 108968 imprime `3510/11+3510/2`, un poids (21,8 kg = 10,5 + 11,3 exactement), un métrage, et les deux étiquettes restent sur le rouleau. Le modèle n'avait qu'un `stock_fini.IDstock_ecru`, et « cet écru est consommé » se lisait à neuf endroits comme « un fini pointe dessus » : un fini sur la seule 3510/11 aurait laissé 3510/2 en stock chez MATEL, double-comptée dans la jauge client et réceptionnable une seconde fois. **Modèle** : table de liaison `stock_fini_source (IDstock_fini_source PK, IDstock_fini, IDstock_ecru)`, une ligne par composant (première pièce incluse), `IDstock_ecru` du fini = première pièce pour le legacy et les lecteurs existants ; `lib/fini-sources.ts` est l'unique propriétaire de « écru consommé » (`consumedEcruIds` / `mergedComponentEcruIds` / `allConsumedEcruIds`, union des deux sources), et les neuf lecteurs l'appellent (stock écru liste + suivi pièce, jauge et deux pools de Clients › Commandes, Rapports, Sous-traitants, garde `releaseShippedPieces` TRM, verrou du dialogue). **UX** (`BatchReceptionDialog`, création seulement) : Ctrl+clic ou case au survol sur une ligne de l'aperçu = fusion avec le rouleau en cours, Maj+clic = fusion de la plage (§44), fusion immédiate (les lignes se replient, les compteurs suivent), « Séparer » la défait, carte `N pièces fusionnées : A + B · poids écru Σ` avec le poids pré-rempli à la somme, « Couper en deux » masqué sur un groupe ; Tricobot forme le groupe lui-même quand `num_piece` contient `+`. Un seul `POST /pieces/fini` avec `IDstock_ecru_sources` (400 composant non affecté, 409 `ecru_deja_recu`, 503 `fusion_indisponible`) ; `fetchPiecesPayload` renvoie `source_ecru_ids` / `source_numeros` (puces sur la carte Réception, verrou de l'onglet Affectés) et `fusion_disponible`. Numéro = forme du BL `3510/11+3510/2` (`mergedNumero`, jumeau web `lib/roll-merge.ts`, repli compact `3510/11+2` au-delà de 20 caractères ; `+` plutôt que `&` : un `&` casse une query string, et la recherche `LIKE` retrouve chaque pièce). Ailleurs : Finis › Stock › Provenance liste les `composants`, une coupe copie les composants sur les morceaux, la surteinture trace chaque composant, les DELETE nettoient la liaison. **Installation de la table — l'histoire qui a coûté l'après-midi** : `CREATE TABLE` via ODBC écrit un `.fic` LOCAL dans le cwd du process, visible de cette seule connexion (toute autre : « Fichier de données inconnu », un nouveau CREATE : « existe déjà ») ; une sonde mono-connexion « prouve » donc à tort que la table marche. La vraie installation : copier `apps/api/hfsql/stock_fini_source.{fic,ndx}` (paire versionnée, pristine) dans le dossier de la base du serveur, **puis redémarrer l'API** (une connexion liste les fichiers à l'ouverture — l'API disait « table missing » après la copie jusqu'au restart). Fait sur le serveur de dev (`C:\PC SOFT\Serveur HFSQL PC-VINCENT-PRO\BDD\mps\`) ; flux complet vérifié par l'API : POST groupé 201, 409 sur re-réception, pièces sorties du stock écru, Suivi pièce et Provenance, DELETE qui libère les deux pièces. **Prod** : dossier `/var/lib/hfsql/mps/` sur `10.10.20.2` (owner `hfsql`, `debian` sudo), commandes dans `etm_deploy` § Step 0 item 5 — **Vincent a approuvé l'installation en prod le 2026-09-14 dans la session de la feature** ; l'API le confirme au démarrage par « stock_fini_source available — grouped rolls enabled ». Tant que la table manque, tout se comporte comme avant et l'affordance est cachée. Le driver refuse `NOT EXISTS` / `NOT IN (SELECT…)` corrélés sur cette table : lectures plates + Sets JS.

## 2026-09-14 — feat/widget-fil (LIVA #1159 — Fil dispo : besoin sans affectation, verrou d’affectation au lancement d’OF)
**#1159 Tableau de bord › État des stocks de fil** : la 9032 (3 000 kg de 029 écru, trois OF lancés côté TRM sur les lots 10553 / 10379) n’apparaissait pas dans le Besoin — le widget ne lisait que `asso_fil_lignecmdsst`, l’affectation ETM, et Pierrot n’avait rien affecté ; le dialogue d’OF TRM choisit ses lots seul. Mesuré en prod le jour même : **9 des 16 couples (ligne, fil) en cours de tricotage chez Tricotage Malterre sans affectation** (9012, 9020, 9024, 9025, 9032). Deux réponses, décidées avec Vincent : (1) le widget remonte aussi la chaîne depuis les OF (`asso_fil_of` → OF → ligne TRM → ligne sst miroir → commande ouverte) et, pour une ligne sans affectation sur ce fil, **dérive la réserve = quantité de ligne × part du fil sur les OF** (pondérée par la quantité d’OF), ligne marquée `source: 'of'` et `180,0 *` dans la colonne Affecté (`lib/fil-etat-besoin.ts`, test sur les chiffres 9032) ; (2) **choisir le fil est la décision de Pierrot** : `lib/affectation-fil-trm.ts` — `POST /of-trm` et `PUT /of-trm/:id/composition` répondent 409 `fil_non_affecte` si un fil (réf + coloris, coloris 0 = joker) de la composition n’est pas affecté sur la ligne ETM ; le lot n’est pas imposé ; une ligne TRM native n’est jamais bloquée ; `GET /of-trm/lookups/composition` renvoie `affectation` et met le lot affecté en tête (défaut du dialogue). Le dialogue TRM (branche TRM `feat/widget-fil`, appariée, landie ensuite) affiche l’encart rouge et désactive « Créer l’OF » ; `apiFetch` TRM porte le corps JSON sur `err.body`. **Prod corrigée le jour même** : huit lignes `asso_fil_lignecmdsst` (ids 922–929) insérées via le bridge pour 9012 / 9020 / 9024 / 9025 après validation de Vincent (la 9032 avait été affectée par Pierrot entre-temps). Doctrine dans `dashboard_widgets.md` § État des stocks de fil ; règle CLAUDE.md § ETM↔TRM bridge. Réponse développeur sur le ticket (statut en_cours).
## 2026-09-14 — feat/bath-edit-tm (LIVA #1156 + audit de performance des écrans de stock)
**#1156 Tombé Métier › Stock** : Pierrot voulait « une sélection multiple par référence et/ou coloris et/ou magasin » — c'est le `SmartSearchInput` à puces de Finis › Stock (§27.2bis du designer), que reçoivent Tombé Métier › Stock (Référence / Coloris / Numéro / Lot / Magasin / N° Cmd / Client / Visiteur / Observations / Défauts) et Fils › Stock (Référence / Coloris / Lot interne / Lot fournisseur / Fournisseur / Emplacement / Magasin / Commentaire). Second point du ticket : la colonne Magasin affichait « — » pour les pièces à l'usine — `IDmagasin = 0`, le « pas de FK » HFSQL, 214 des 762 écru disponibles ; normalisé en « Malterre » dans le `select` des requêtes liste ET détail (`withDefaultMagasin`, même libellé que Finis › Stock), Fils › Stock pareil sur `IDMagasin`. **Audit de performance** (Vincent, même jour) : la requête liste est bimodale côté serveur (~450 ms ou ~2,5 s pour le même SQL, serveur inactif ; prod 0,9 s) — toute comparaison de forme SQL se fait entrelacée dans un même run (`probe-1156-ecru-perf6/9/10.ts`) ; un seul `SELECT` sans JOIN, libellés par trois lookups `IN` plats (`attachEcruLabels`) ; `fetchDefectsByEcru` (8 routes) en tranches de 50 — `defaut_qualite.reference` est une chaîne, une liste `IN` de 766 littéraux scannait les 12 k défauts — et accents réparés en un `repairAliased` groupé : hydratation 620 → 230 ms. Côté navigateur, les quatre écrans de stock rendaient la table ET la liste de cartes mobile (1 524 éléments pour 762 pièces, 34 k nœuds DOM) ; `useMediaQuery(MD_UP)` ne monte plus qu'une branche (12,6 k nœuds, tâches longues ~700 → ~380 ms en dev). Écarté après A/B : lire le mémo `observations` dans un second `SELECT` limité aux lignes non vides (double la note en état lent). Règles dans CLAUDE.md (§ HFSQL, § React, § 7), histoire dans `hfsql_odbc.md` § Footguns et `screen_notes.md` § 7. Réponse développeur postée sur #1156 (en_cours, corrigé en 0.3.5 côté tracker). ⚠️ Le harnais e2e (`apps/web/e2e`) ne mocke ni `/api/tickets*` ni `/api/user-profiles*` depuis août : ses specs échouent sur `unmatched` avant toute comparaison d'images.

## 2026-09-11 — feat/client-ref-fini (LIVA #1155 + refonte de la fiche Finis › Références)
**#1155 — « qui a déjà commandé cette référence »** : `GET /references-fini/:id/clients` (`lib/clients-ref-fini.ts`, agrégateur pur sous Vitest) porte le `TABLE_Client` de l'ancien `FI_Ref_Fini` — lignes `TYPE = 2` de la réf → en-têtes `commande_client` **`IDsociete = 1`** par paquets de 200 (l'ensemble des en-têtes EST le filtre de partition) → noms clients ; une ligne par client **triée par volume** (Σ Ml, puis Σ Kg, puis nom — décision Vincent), avec nombre de commandes distinctes et dernière commande. Affichée dans une carte « Clients » de l'onglet Informations du panneau droit, en lecture seule. **Refonte de la fiche** au fil des retours du même jour : le panneau central devient un **Classeur §39** (onglets Spécifications / Coloris / Traitements, compteurs pris dans le détail déjà chargé) ; Spécifications = grille de tuiles (six tuiles techniques, quatre de stabilité, tuile « Contrôles à réception » montrant TOUJOURS les trois drapeaux en pastilles on/off, puis conditionnement / observations / observation technique / description commerciale en tuiles pleine largeur `whitespace-pre-line` — chaque conditionnement du catalogue est multi-ligne CRLF, la saisie passe en textarea), la référence écru en tête avec l'icône Tombé Métier ; Coloris = tableau (Coloris · Teinture · Note · Stock) avec le **stock vivant par coloris** (`stock_fini` GROUP BY `IDColoris`, même prédicat « non expédié » que le total) et un filtre au-delà de 8 coloris ; Traitements = chaîne numérotée dans `traitement.ordre` (`loadRefFiniTraitements` renvoie désormais `ordre` + `observation`) avec ✕ et emplacement « + Ajouter » en édition ; Stock actuel et référence écru quittent le centre pour la barre latérale / la tuile écru. Les cartes Stabilité, Observations et Stock du centre sont supprimées, ainsi que les en-têtes repliables des trois restantes. ⚠️ `ref_fini_colori` n'a PAS de colonne `IDclient` (l'extraction du `MPS.xdd` débordait sur la table suivante — 500 sur toute la fiche en dev, tempête de respawn sur Linux) et `finition` est un code numérique : ni l'un ni l'autre n'est lu. Réponse développeur postée sur le ticket (statut en_cours, corrigé en 0.3.4 côté tracker).

## 2026-09-11 — feat/donation (#1154 — attaching a piece to a donation is the stock exit)
**#1154 Clients › Commandes › Donation** : order 3868 had six fini rolls attached (3041/4 among them) still listed in Finis › Stock. The picker wrote `IDcommande_donation` alone, while Finis › Stock defines "in stock" as no avis AND état 3; the legacy `FEN_Ligne_Donation` stamped état 4 « Expédié » on attach (877 of 888 donation rolls in prod: état 4, no expedition — a donation never has an avis). New `lib/donation-pieces.ts` (pure SQL builders, 15 tests): fini attach = FK + état 4 in one UPDATE; detach (uncheck or DELETE of the commande) = état 4 → 3 only on rolls still in 4, then FK → 0 (same order as `unshipFiniRolls`). Écru: FK only, and Tombé Métier › Stock now excludes donation écru on every tab like the legacy `FI_Stock_TM_ETM`. Guard + repair `scripts/check-donation-etat.ts [--repair]`, run on prod the same day (10 rolls of orders 3840/3868 repaired). Rule in CLAUDE.md § Data semantics, story in `screen_notes.md` § 3.

## 2026-09-11 — feat/debug-1 (LIVA #1148, page blanche sur la facture 9228)
**Clients › Facturation — première page blanche** : la table des lignes de la facture 9228 (six lignes, désignations sur quatre à cinq lignes) tenait sur la page 1 à 3,5 pt près, mais son `marginBottom: 16` dépassait la réserve du pied de page ; le paginateur de react-pdf 4.5 (`shouldBreak` : `!shouldSplit && endOfPresence > height`, avec `endOfPresence = top + hauteur + marginBottom`) déplace alors le bloc ENTIER au lieu de couper entre deux lignes → page 1 vide sous les cartes, totaux page 2, RIB page 3. Correctif : plus de marge basse sur la table, l'écart passe en `marginTop` du bloc des totaux — facture (16 pt), commande client, devis, commande fournisseur, commande sous-traitant (8 pt) ; `RapportFreintePdf` / `SoumissionLotPdf` gardent la leur, à porter au prochain passage. Garde `FacturePdf.test.ts` (rend la forme exacte de la 9228, compte les objets `/Type /Page`, attend 2 pages — 3 avant) ; `dump-facture-pdf.ts` gagne la variante `-1148` et un `rasterDir` optionnel. Vérifié sur données dev : la facture 8193 (24/01/2025) passait de 3 à 2 pages, seule différence sur les 1 000 dernières factures. Piège et méthode de diagnostic (trace dans `splitNodes` de `@react-pdf/layout`) dans `claude_doc/pdf_email.md` § Pagination.

## 2026-09-11 — feat/debug-1 (API half of two TRM tickets, paired with TRM-debug-1)
**#1147 Production › OF — stock de fil** : `GET /of-trm/:id` sommait tous les lots de la paire (fil, coloris) dans `pair_stock` et calculait « Réalisable » dessus (2 226 Kg affichés pour un lot de 380 Kg). L'unité est le **lot choisi** : `lib/realisable-fil-trm.ts` (`realisableSurLots`, stock ÷ Σ % des lignes nourries par le lot, min sur les lots), `pair_stock` supprimé, le web en porte une copie verbatim testée contre ce fichier. **#1151 Clients › Commandes — prix** : chaque ligne du détail `/commandes-trm/:id` porte `tarif` (coût, prix calculé = coût + 30 %, base de la fiche, retenu, suggéré, qui a tarifé) pour le tooltip de la pastille de marge. **Une seule règle de tarif** (décision Vincent 2026-09-11) : `max(coût / 0,7, base)`, la base retenue telle quelle — `/lookups/line-price` rejoint `trmLinePrix`, `TrmBaseRole` et la règle « cost-floor » du 2026-08-26 sont retirés (elle donnait 3,29 € là où le pont ETM écrivait 2,30 € pour la même fiche). `retain()` compare désormais la base au coût **margé** (elle disait « base » en portant coût / 0,7). **Bug prod** : `loadRefEcruMachineCodes` lisait `ref_ecru.diamètre` par une liste d'orthographes et trouvait 0 sur le pont Linux → 0 aiguille, ligne « Changement aiguilles » à 0 € sur toutes les références (coût sous-estimé ~3 %) ; lecture par préfixe (`pickVal`), quatrième cas du piège dans `accented-keys.ts`. Le web TRM (`TRM-debug-1`) doit se déployer **après** cette API.
