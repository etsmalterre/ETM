...
| `Attente_Delai` → `En_Cours` | `PUT /lignes/:lineId` with `date_livraison` in the patch | When the row's current sstatut is exactly `Attente_Delai`, the same UPDATE that changes `date_livraison` also sets `sstatut = 'En_Cours'`. Lines still in `Non_Envoye` keep their status (the bon de commande hasn't been sent yet — the date is hypothetical). An explicit `sstatut` in the PUT body always wins over this auto-flip. **Same transition from the TRM side** since LIVA #1123 (2026-09-07): `PUT /commandes-trm/lignes/:id/delai` — the sister company announcing its knitting date — applies the identical rule (and the capture-once `date_delai`) to the sst line behind a mirrored TRM line. Both routes call `sstDelaiSets()` in `lib/sst-shared.ts`; change the rule there, once. |
