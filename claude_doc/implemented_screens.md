...
- **TRM order lines add `TYPE = 4` = rectiligne** (cols / bandes, `ref_rectiligne` + `coloris_rectiligne` — 175 legacy rows, all mirrors of ETM sst type-4 lines on the "Ets Malterre" customer). Until 2026-09-23 this read them as « confection » against the écru catalog, whose ids collide (R006-38 showed as « LTP02 »); the historique now labels them from the rectiligne tables (LIVA #1185, `lib/sst-line-kind.ts`).
