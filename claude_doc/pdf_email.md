...
- ⚠️ **A `marginBottom` on a wrapping block is part of its "presence" for the paginator — a table that fits page 1 by a few points but whose margin does not is pushed WHOLE to the next page, leaving page 1 blank under the cards** (LIVA #1148, facture 9228: six lines, table ending 3,5 pt above the reserve, `marginBottom: 16` → three pages, page 1 empty). The rule is `shouldBreak` in `@react-pdf/layout` 4.5: `!shouldSplit && endOfPresence > height && breakingImprovesPresence`, where `endOfPresence = top + height + marginBottom + minPresenceAhead` — so it never splits the rows, it moves the block. Doctrine: **no `marginBottom` on a lines table; the gap goes on the following block's `marginTop`** (facture, commande client, devis, commande fournisseur, commande sous-traitant — all five done 2026-09-11; `RapportFreintePdf` / `SoumissionLotPdf` still carry one, port when touched). Pinned by `FacturePdf.test.ts` (renders the 9228 shape, counts `/Type /Page` objects — pdfkit writes dictionaries uncompressed, only streams are deflated — expects 2 pages; 3 before the fix). Harness: `dump-facture-pdf.ts [out] [rasterDir]` writes the `-1148` variant. Diagnosing a pagination surprise: temporarily `console.error` in the `shouldBreak` branch of `splitNodes` in `node_modules/.pnpm/@react-pdf+layout@*/…/lib/index.js` (backup, run, restore) — it prints top / height / limit / endOfPresence per moved node in one run.
...
- ⚠️ **Accented CAPITALS and letter-spacing lose their accent** (react-pdf + the bundled Lato, every weight — the font does have the glyphs; seen 2026-09-25, #1213): « CARACTÉRISTIQUES » printed « CARACTERISTIQUES » on the old fiche technique, and `letterSpacing` pushes the acute of a lowercase é off its letter. Section titles go in sentence case with no letter-spacing; an uppercase + tracked label must be an accent-free word (POIDS, LAIZE UTILE). Seen in pdf.js rasters only — not re-checked in another viewer; the root cause is not fixed.
...
- **Dymo 89 × 36 tags — two centring doctrines, on purpose.** The LabelWriter's head stops printing ~234 pt in (≈ 82,5 mm of the 89; measured 2026-08-27, `EtiquetteEcruPdf.tsx`). The *internal* écru tag centres in the **printable band** (5 pt left / 26 pt right). The *client-facing* ref_fini tag (`EtiquetteRefFiniPdf.tsx`) centres on the **physical label** — equal padding both sides, content narrowed to 210 pt so the QR's right edge still clears the head limit; the first print read shifted left (user, 2026-09-08). Both invariants are pinned by `EtiquetteRefFiniPdf.test.ts` (`LAYOUT`). Do not "harmonise" one onto the other. Harness: `dump-etiquette-ref-fini.ts <id|longest> [outDir] [rasterDir]` (rasterDir = a temp dir holding `pdf-to-img`).
